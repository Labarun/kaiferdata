/**
 * Staff Transactions Page — Read-only payment record search
 */
...
export default function StaffTransactionsPage() {
  ...
  const fetchRecords = useCallback(async () => {
    setLoading(true);
    let query = supabase.from("payment_records").select("*").order("created_at", { ascending: false }).limit(100);
    if (statusFilter !== "all") query = query.eq("status", statusFilter as "pending" | "verified" | "failed" | "reversed");
    if (search.trim()) {
      query = query.or(`provider_reference.ilike.%${search.trim()}%,internal_reference.ilike.%${search.trim()}%`);
    }
  ...
  return (
    <div className="animate-fade-in space-y-4">
      <PageHeader title="Transactions" description="Payment records (read-only)" />

      <div className="flex flex-col sm:flex-row gap-2">
        <div className="relative flex-1">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground/50" />
          <Input placeholder="Search by reference…" value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 h-10 text-sm" onKeyDown={(e) => e.key === "Enter" && fetchRecords()} />
        </div>
        ...
        <Card>
          <CardContent className="p-3">
            ...
            <div className="flex flex-wrap gap-1">
              ...
              {statusFilter !== "all" && (
                <button onClick={() => setStatusFilter("all")} className="text-[11px] text-destructive hover:underline ml-2 flex items-center gap-1"><X className="h-3 w-3" /> Clear</button>
              )}
            </div>
          </CardContent>
        </Card>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border/50">
                {["Internal Ref", "Provider Ref", "Amount", "Status", "Email", "Verified", ""].map((h) => (
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7} className="text-center py-8 text-xs text-muted-foreground">No records found.</td></tr>
              ) : (
                records.map((r) => (
                  <tr key={r.id as string} className="border-b border-border/30 hover:bg-muted/20 transition-colors">
                    <td className="px-3 py-2.5">
                      <Link to={`/staff/transactions/${r.id}`} className="font-mono text-[12px] font-medium text-primary hover:underline">{r.internal_reference as string}</Link>
                    </td>
                    ...
                    <td className="px-3 py-2.5 text-[11px] text-muted-foreground whitespace-nowrap">{r.verified_at ? new Date(r.verified_at as string).toLocaleDateString() : "—"}</td>
                    <td className="px-3 py-2.5"><Link to={`/staff/transactions/${r.id}`}><ChevronRight className="h-4 w-4 text-muted-foreground/40" /></Link></td>
                  </tr>
                ))
              )}
            </tbody>
