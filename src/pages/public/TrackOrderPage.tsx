/**
 * TrackOrderPage - Premium order tracking with glass search
 */
    ...
    <div className="min-h-[70vh]">
      ...
      <div className="bg-hero-gradient">
        <div className="container py-8 sm:py-10 text-center">
          ...
          <h1 className="text-xl sm:text-2xl font-extrabold text-hero-foreground">Track Your Order</h1>
      ...
      <div className="container py-5 sm:py-6">
        <div className="max-w-md mx-auto">
          ...
          <div className="glass rounded-2xl p-4 sm:p-5 mb-5">
            <div className="space-y-3">
              <Input
                ...
                className="h-12 font-mono text-sm rounded-xl font-semibold text-center"
                maxLength={30}
          ...
          {/* Result */}
          {result && (
            <div className="rounded-xl border border-border/60 bg-card p-4 sm:p-5 animate-fade-in space-y-3 text-sm">
              <div className="flex items-center gap-2 mb-1">
                <Package className="h-4 w-4 text-primary" />
                <span className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Order Details</span>
              </div>
...
function TrackRow({ label, value, mono, bold, badge }: {
  ...
  return (
    <div className="flex items-center justify-between gap-2">
      ...
      {badge ? (
        <span className="text-[11px] font-bold capitalize px-2 py-0.5 rounded-full bg-primary/10 text-primary">
          {value}
        </span>
      ) : (
        <span className={`text-sm text-foreground ${mono ? "font-mono" : ""} ${bold ? "font-extrabold text-primary" : "font-semibold"}`}>
          {value}
        </span>
      )}
    </div>
