-- ============================================================
-- Optimize list_public_packages RPC
-- Move auth checks to a CTE so they are evaluated only once 
-- instead of repeatedly for every row in the data_packages table, 
-- massively improving load times on the User Dashboard.
-- ============================================================
...
WITH user_status AS (
  SELECT 
    ...
    ELSE (
      EXISTS (SELECT 1 FROM public.agent_profiles WHERE user_id = auth.uid() AND status = 'active')
      OR public.has_role(auth.uid(), 'admin')
    ) END AS is_agent_or_admin
)
...
CASE
WHEN us.is_agent_or_admin THEN dp.agent_base_price
ELSE 0
