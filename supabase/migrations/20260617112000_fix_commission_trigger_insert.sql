-- ============================================================
-- Migration: Fix Commission Trigger to fire on INSERT + support 'queued' status
-- ============================================================

-- 1. Drop the existing trigger
DROP TRIGGER IF EXISTS trg_orders_commission_on_delivered ON public.orders;

-- 2. Update the trigger function to support 'queued' status
...
BEGIN
  ...
  -- Query db for base price securely first, fall back to payload
...
-- 3. Re-create the trigger for BOTH INSERT and UPDATE OF status
CREATE TRIGGER trg_orders_commission_on_delivered
AFTER INSERT OR UPDATE OF status ON public.orders
FOR EACH ROW
EXECUTE FUNCTION public.handle_order_delivered_commission();

-- 4. Run backfill for orders that were never credited (including paid, processing, delivered, queued)
DO $$
DECLARE
  o RECORD;
  v_referral jsonb;
