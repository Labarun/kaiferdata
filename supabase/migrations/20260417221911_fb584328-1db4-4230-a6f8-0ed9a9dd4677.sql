...
DECLARE
  ...
  v_safety text;
BEGIN
  -- ── 0. Safety guard: respect global order-submission toggle
  ...
  -- ── 2. Re-resolve package server-side (zero-trust pricing)
  ...
  v_public_order_id := 'KD-ORD-' || upper(substr(replace(gen_random_uuid()::text,'-',''), 1, 10));

  -- ── 6. Create the order first (so we can link txn → order)
  ...
  -- ── 7. Atomic debit (idempotent on reference — order id makes it unique)
...
-- Allow authenticated users to call it (function still re-checks auth via _user_id parameter — but we add a SECURITY guard inline)
REVOKE ALL ON FUNCTION public.purchase_with_wallet_atomic(uuid, uuid, text, text, text, text, text) FROM PUBLIC;
