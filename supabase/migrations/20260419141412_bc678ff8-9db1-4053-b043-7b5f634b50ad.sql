...
-- ─────────────────────────────────────────────────────────────────────────
-- 1. Optional contact phone on agent_profiles (storefront-facing)
-- ─────────────────────────────────────────────────────────────────────────
ALTER TABLE public.agent_profiles
  ADD COLUMN IF NOT EXISTS contact_phone text;

COMMENT ON COLUMN public.agent_profiles.contact_phone IS
  'Public-facing WhatsApp/phone number for the agent storefront. Optional.';

-- Allow agent owner to update their own contact_phone via existing
-- "Owner updates store details" UPDATE policy (already covers all columns).

-- ─────────────────────────────────────────────────────────────────────────
-- 2. Replace handle_order_delivered_commission with a version that also
--    credits as soon as the order is paid (still fully idempotent).
-- ─────────────────────────────────────────────────────────────────────────
...
BEGIN
  -- Only act when status reaches a "money is locked in" state and we
  -- haven't acted before for this status transition.
  -- Eligible target statuses:  paid, processing, delivered.
  IF NEW.status NOT IN ('paid','processing','delivered') THEN
    RETURN NEW;
  END IF;

  -- Skip if status didn't actually change (UPDATE that touched other cols)
  IF TG_OP = 'UPDATE' AND OLD.status = NEW.status THEN
    RETURN NEW;
  END IF;

  -- For UPDATE we credit on first transition into paid/processing/delivered.
  -- For INSERT (status='paid' from wallet purchase) we also credit.
  IF TG_OP = 'INSERT' THEN
    v_should_credit := true;
  ELSIF TG_OP = 'UPDATE' THEN
    -- Only the FIRST entry into a creditable state should trigger.
  ...
  -- Compute profit: prefer pricing snapshot, fall back to legacy %
  ...
  -- Find or create the agent's earnings wallet
  ...
  -- Credit the earnings wallet (idempotent on reference)
...
-- Re-bind the trigger so it now runs on INSERT and UPDATE, but only once
-- per order on first entry into a creditable state. Existing trigger may
-- be UPDATE-only; replace cleanly.
DROP TRIGGER IF EXISTS trg_orders_credit_commission ON public.orders;
CREATE TRIGGER trg_orders_credit_commission
AFTER INSERT OR UPDATE OF status ON public.orders
FOR EACH ROW
EXECUTE FUNCTION public.handle_order_delivered_commission();

-- ─────────────────────────────────────────────────────────────────────────
-- 3. One-time backfill: credit commissions for already-paid referred
