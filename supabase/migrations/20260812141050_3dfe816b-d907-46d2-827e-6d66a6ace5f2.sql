...
BEGIN
  ...
  -- Total commission paid/pending to agents (filtered by order creation date for the timeframe)
...
-- Security lockdown: revoke from PUBLIC and grant only to authenticated roles
...
GRANT EXECUTE ON FUNCTION public.get_sales_trends(int) TO service_role;
GRANT EXECUTE ON FUNCTION public.get_admin_profit_stats(text) TO service_role;
